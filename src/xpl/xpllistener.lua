...
listener = {
    ...
    -- @return the IP address now used by the listener and to be used in hbeat messages for the 'remote-ip' key.
    ...
    -- @return the network port now used by the listener and to be used in hbeat messages for the 'port' key.
    getport = function ()
        return port
    end,
    ...
    -- @param status if <code>nil</code> then all devices will be added to the return table, otherwise only the devices with a status property matching this status will be returned.
...
-- @class table
-- @name events
-- @field newmessage event to indicate a new message has arrived. The message will be passed as an argument to the event handler.
-- @field networkchange event to indicate that the network state changed (ip address, connection lost/restored, etc.). The <code>newState</code> and <code>oldState</code> will be passed as arguments to the event handler (see 'NetCheck' documentation for details on the <code>xxxState</code> format)
-- @see subscribe
